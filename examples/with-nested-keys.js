    ...
    resolvers: {
      Query: {
        ...
        authors: (_, args) => {
          ...
          // sort by first name for no reason, results will be matched properly
          authors.sort((a, b) => a.name.firstName > b.name.firstName ? 1 : -1)
...
async function test () {
  ...
  const queries = [
    ...
    `{ getReviewBookByIds(bookIds: [2,3,4,5]) {
      title
      author {
        name { lastName }
      }
