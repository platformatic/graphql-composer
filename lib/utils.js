...
function collectPlainArgs (args, nodeArguments, info, types) {
  ...
  // graphql-js fills a value the client did not send from the argument's default, so there is
  // no node for it; leave it out and the subgraph applies the same default
  if (nodeArguments === undefined) {
    return
  }
