...
const { isObject, defaultOnError, createDefaultArgsAdapter } = require('./utils')
const nullLogger = require('abstract-logging')

const DEFAULT_COMPOSE_ENDPOINT = '/.well-known/graphql-composition'
const DEFAULT_GRAPHQL_ENDPOINT = '/graphql'
const CONFLICT_STRATEGIES = ['first', 'last', 'route', 'error']
...
function validateComposerOptions (options) {
  ...
  // unset keeps the merge as it was before the option existed, see Composer#buildSharedFieldResolvers
  validatedOptions.onConflict = options.onConflict
  if (validatedOptions.onConflict !== undefined && !CONFLICT_STRATEGIES.includes(validatedOptions.onConflict)) {
    throw new TypeError(`onConflict must be one of ${CONFLICT_STRATEGIES.map(s => `"${s}"`).join(', ')}`)
  }
  ...
  if (options.composeEndpointMissCache instanceof Map) {
    validatedOptions.composeEndpointMissCache = options.composeEndpointMissCache
  }
