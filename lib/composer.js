...
class Composer {
  ...
  async #runFollowupQueries (ctx, merged) {
    ...
    for (const followup of ctx.followups.values()) {
      // set solved followup
      // !! HERE !!
      // assumes all the result in a list are the same
      if (Array.isArray(merged)) {
        merged = merged[0]
      }
      // need to map folloup -> resolved
...
function mergeResults (query, partialResult, response) {
  ...
  if (mergeAt === null) {
    ...
    console.log(' !!! ***TODO*** mergeResults, array', result)

    for (let i = 0; i < result.length; i++) {
      const r = result[i]
      // TODO! not by index, match by source id
