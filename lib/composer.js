...
const { isObject, keySelection } = require('./utils')
const {
  validateArray,
  validateFunction,
  validateObject,
...
class Composer {
    ...
    for (let i = 0; i < subgraphs.length; ++i) {
      ...
      if (isObject(entities)) {
        ...
        for (let j = 0; j < entityNames.length; ++j) {
          ...
          if (args && !referenceListResolverName) {
            throw new TypeError(`subgraphs[${i}].entities.${name}.referenceListResolverName must be provided`)
          }

          if (referenceListResolverName && !args) {
            throw new TypeError(`subgraphs[${i}].entities.${name}.args must be provided`)
          }

          if (args) {
            validateFunction(args, `subgraphs[${i}].entities.${name}.args`)
          }
          if (referenceListResolverName) {
  ...
  /**
   * is followup already solved by query
...
function mergeResults (query, partialResult, response) {
  ...
  // TODO refactor to mergeResultData fn
  if (mergedPartial === null) {
    ...
    // TODO
    // if (!query.keys[0]) {
    // onError('missing key for ... in ... ')
    // throw unable to merge results, missing entity key
    // }

    // TODO support composite key
    const key = keySelection(query.keys[0].field)
    const originKey = keySelection(query.originKeys[0].field)

    for (let i = 0; i < result.length; i++) {
      const r = result[i]
      const rowKey = r[key]
      const row = mergedPartial.find(r => r[originKey] === rowKey)
      if (!row) { continue }
