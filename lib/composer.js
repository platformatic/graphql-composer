...
const { isObject, createDefaultArgsAdapter, traverseResult, schemaTypeName } = require('./utils')
const {
  validateArray,
  validateFunction,
  validateObject,
    ...
    for (let i = 0; i < subgraphs.length; ++i) {
      ...
      if (isObject(entities)) {
        ...
        for (let j = 0; j < entityNames.length; ++j) {
          ...
          //     // TODO! as: cant referer to self, must be unique per entity
          //   } else {
          //     keys[k].type = name
          //   }
          // }
          ...
          if (resolver?.argsAdapter) {
            validateFunction(resolver.argsAdapter, `subgraphs[${i}].entities.${name}.resolver.argsAdapter`)
          }

          if (resolver?.name) {
            validateString(resolver.name, `subgraphs[${i}].entities.${name}.resolver.name`)
            if (!resolver?.argsAdapter) {
      ...
      // TODO! check subgraphName is unique
