      ...
      if (isObject(entities)) {
        ...
        for (let j = 0; j < entityNames.length; ++j) {
          ...
          if (!Array.isArray(keys) && keys.length < 1) {
            throw new TypeError(`subgraphs[${i}].entities.${name}.keys must be an array with at least one key`)
          }
          ...
          if (referenceListResolverName) {
            ...
            if (!argsAdapter) {
              argsAdapter = defaultArgsAdapter ?? createDefaultArgsAdapter(name, keys)
            }
          }
