...
class Composer {
  ...
  /**
   * A Query or Mutation field published by more than one subgraph is routed by an enum argument:
   * every subgraph declares the argument with the same enum type and its own values, the merged
   * schema carries the union of the values (see `mergeTypes`), and the value a client sends picks
   * the subgraph. Without this pass the field silently kept the last subgraph's resolver while the
   * enum kept the first subgraph's values, so a call could validate against one subgraph and run
   * against another.
   *
   * Composition fails when a shared field has no such argument, or when a value is declared by
   * more than one subgraph: either way there is no single subgraph that owns the call.
   */
  buildSharedFieldResolvers () {
    const mainTypeNames = new Set()
    for (const mainType of this.mainTypes.values()) {
      mainTypeNames.add(this.schemaOptions[mainType].name)
    }

    for (const fieldId of Object.keys(this.fields)) {
      const typeName = fieldId.slice(0, fieldId.indexOf('.'))
      if (!mainTypeNames.has(typeName)) { continue }

      const subgraphNames = Object.keys(this.fields[fieldId]).filter(s => s !== COMPOSER_SUBGRAPH_NAME)
      if (subgraphNames.length < 2) { continue }

      const { argName, routes } = this.sharedFieldDiscriminator(fieldId, subgraphNames)
      this.logger.debug({ fieldId, argName, routes: Object.fromEntries(routes) }, 'route shared field by enum argument')

      this.resolvers[typeName][fieldId.slice(typeName.length + 1)] = this.createRoutedResolver({ typeName, fieldId, argName, routes })
    }
  }

  /**
   * find the one argument of a shared field that routes it: an enum, of the same type name in
   * every subgraph, whose values are declared by exactly one subgraph each
   * @returns {{ argName: string, routes: Map<string, string> }} routes: enum value -> subgraph name
   */
  sharedFieldDiscriminator (fieldId, subgraphNames) {
    const candidates = []
    const firstField = this.fields[fieldId][subgraphNames[0]].src

    for (const firstArg of firstField.args ?? []) {
      const enumName = enumArgTypeName(firstArg.type)
      ...
      for (const subgraphName of subgraphNames) {
        const arg = this.fields[fieldId][subgraphName].src.args?.find(a => a.name === firstArg.name)
        if (!arg || enumArgTypeName(arg.type) !== enumName) {
          disjoint = false
          break
        }

        const values = this.types[enumName]?.[subgraphName]?.src.enumValues ?? []
      ...
      if (disjoint && routes.size > 0) {
        candidates.push({ argName: firstArg.name, routes })
      }
    }
    ...
    if (candidates.length === 0) {
      throw new Error(`Cannot compose ${where}: no enum argument with a distinct set of values per subgraph to route by`)
    }
