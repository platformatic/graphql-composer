...
class Composer {
  ...
  constructor (options) {
    const {
      ...
      subgraphs = [],
      subscriptions = defaultPubSubHandler
    } = options
  ...
  #createSubscription ({ type, field, fieldName, subgraph, pubsub }) {
    return async (parent, args, contextValue, info) => {
      ...
      // TODO(cjihrig): Need to handle cleaning up and closing the connection.
      client.on('message', async (msg) => {
        // TODO(cjihrig): Run any follow up queries to send full data back.
        await pubsub.publish(contextValue, topic, msg)
      })
      ...
      topic = await client.subscribe(text)
