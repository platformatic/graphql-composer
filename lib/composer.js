...
class Composer {
  constructor (options = {}) {
    ...
    // the Query and Mutation type names actually seen in the subgraphs
    this.mainTypeNames = new Set()
  ...
  async compose () {
    ...
    this.buildSharedFieldResolvers()
  }
  ...
  buildMergedSchema () {
    ...
    for (let i = 0; i < typeNames.length; ++i) {
      ...
      for (let j = 0; j < subgraphNames.length; ++j) {
        ...
        // TODO add option to rename, mask or hide conflicting fields
        this.mergedSchema.types.set(typeName, t ? mergeTypes(t, type, this.onConflict) : objectDeepClone(type))
      }
    }
  ...
  mergeSchema ({ __schema: schema, subgraphName }) {
    ...
    for (let i = 0; i < schema.types.length; ++i) {
      ...
      if (mainType) {
        ...
        this.mainTypeNames.add(typeName)
      }
  ...
  /**
   * A Query or Mutation field published by more than one subgraph is resolved according to the
   * `onConflict` strategy:
   * - "error": composition fails, naming the field and the subgraphs
   * - "first" / "last": that subgraph's resolver answers, and a warning names the others
   * - "route": every subgraph declares the field with the same signature and an enum argument
   *   whose values are disjoint per subgraph; the merged enum is the union (see `mergeTypes`) and
   *   the value a client passes picks the subgraph
   * - unset: as before the option existed, the last subgraph's resolver answers while a shared
   *   enum keeps the first subgraph's values, so a call can validate against one subgraph and run
   *   against another; a warning names the field and points at the option
   */
  buildSharedFieldResolvers () {
    for (const fieldId of Object.keys(this.fields)) {
      const typeName = fieldId.slice(0, fieldId.indexOf('.'))
      if (!this.mainTypeNames.has(typeName)) { continue }

      const subgraphNames = Object.keys(this.fields[fieldId]).filter(s => s !== COMPOSER_SUBGRAPH_NAME)
      if (subgraphNames.length < 2) { continue }

      const fieldName = fieldId.slice(typeName.length + 1)
      const where = `${fieldId} is published by subgraphs ${subgraphNames.join(', ')}`

      if (this.onConflict === undefined) {
        this.logger.warn({ fieldId, subgraphs: subgraphNames }, `${where}: resolved by the last subgraph, set onConflict to choose how`)
        continue
      }

      if (this.onConflict === 'error') {
        throw new Error(`Cannot compose ${where}: set onConflict to "route", "first" or "last" to resolve it`)
      }

      if (this.onConflict === 'first' || this.onConflict === 'last') {
        const winner = this.onConflict === 'first' ? subgraphNames[0] : subgraphNames[subgraphNames.length - 1]
        this.logger.warn({ fieldId, subgraphs: subgraphNames, winner }, `${where}: resolved by subgraph ${winner} (onConflict: ${this.onConflict})`)
        this.resolvers[typeName][fieldName] = this.fields[fieldId][winner].resolver
        continue
      }

      this.assertSameSignature(fieldId, subgraphNames, where)
      const { argName, routes } = this.sharedFieldDiscriminator(fieldId, subgraphNames, where)
      this.assertSameDefaults(fieldId, subgraphNames, where, argName)
      this.logger.debug({ fieldId, argName, routes: Object.fromEntries(routes) }, 'route shared field by enum argument')

      this.resolvers[typeName][fieldName] = this.createRoutedResolver({ typeName, fieldId, argName, routes })
    }
  }

  /**
   * a routed field is validated once, against the merged schema, then sent to one subgraph:
   * the subgraphs must agree on the return type and on every argument's type, or a call that
   * passes validation can fail in the subgraph that receives it
   */
  assertSameSignature (fieldId, subgraphNames, where) {
    const signatures = new Map()
    for (const subgraphName of subgraphNames) {
      signatures.set(subgraphName, printFieldSignature(this.fields[fieldId][subgraphName].src))
    }

    if (new Set(signatures.values()).size > 1) {
      const listed = Array.from(signatures, ([s, sig]) => `${s}: ${sig}`).join('; ')
      throw new Error(`Cannot compose ${where}: the signatures differ (${listed})`)
    }
  }

  /**
   * an argument the client leaves out is filled from the merged schema's default, which is the
   * first subgraph's, and sent without a value so the receiving subgraph applies its own: the
   * defaults must agree. The routing argument is the exception: each subgraph can only default
   * it to a value it declares, and a call left to the merged default is routed to that very
   * subgraph.
   */
  assertSameDefaults (fieldId, subgraphNames, where, routingArgName) {
    const defaults = new Map()
    for (const subgraphName of subgraphNames) {
      const args = (this.fields[fieldId][subgraphName].src.args ?? [])
        .filter(arg => arg.name !== routingArgName && arg.defaultValue != null)
        .map(arg => `${arg.name} = ${arg.defaultValue}`)
        .sort()
      defaults.set(subgraphName, `(${args.join(', ')})`)
    }

    if (new Set(defaults.values()).size > 1) {
      const listed = Array.from(defaults, ([s, d]) => `${s}: ${d}`).join('; ')
      throw new Error(`Cannot compose ${where}: the argument defaults differ (${listed})`)
    }
  }

  /**
   * find the one argument of a shared field that routes it: an enum, declared as `Enum!` or with
   * a default value, whose values are declared by exactly one subgraph each
   * @returns {{ argName: string, routes: Map<string, string> }} routes: enum value -> subgraph name
   */
  sharedFieldDiscriminator (fieldId, subgraphNames, where) {
    const candidates = []
    const optional = []
    // the signatures are identical at this point, so the first subgraph's arguments stand for all
    const args = this.fields[fieldId][subgraphNames[0]].src.args ?? []

    for (const arg of args) {
      const enumName = enumArgTypeName(arg.type)
      if (!enumName) { continue }

      // a client may omit a nullable argument without a default, and then nothing routes the call
      if (arg.type.kind !== 'NON_NULL' && arg.defaultValue == null) {
      ...
      let disjoint = true
      for (const subgraphName of subgraphNames) {
        for (const { name } of this.types[enumName][subgraphName].src.enumValues) {
          if (routes.has(name)) {
            disjoint = false
            break
          }
          routes.set(name, subgraphName)
        }
        if (!disjoint) { break }
      }

      if (disjoint && routes.size > 0) {
        candidates.push({ argName: arg.name, enumName, routes })
      }
    }

    if (candidates.length > 1) {
      throw new Error(`Cannot compose ${where}: more than one enum argument could route it (${candidates.map(c => c.argName).join(', ')})`)
    }

    if (candidates.length === 0) {
      const hint = optional.length > 0
        ? `; ${optional.map(name => `"${name}"`).join(', ')} is a nullable enum argument without a default value, declare it as non-null or give it a default to route by it`
        : ''
      throw new Error(`Cannot compose ${where}: no enum argument with a distinct set of values per subgraph to route by${hint}`)
    }

    // the merged enum is the union over EVERY subgraph declaring it, not only the field's
    // publishers, and a value nobody publishing the field declares would pass validation
    // and then have nowhere to go
    const { argName, enumName, routes } = candidates[0]
    const unroutable = []
    for (const subgraphName of Object.keys(this.types[enumName])) {
      if (subgraphName === COMPOSER_SUBGRAPH_NAME || subgraphNames.includes(subgraphName)) { continue }
      for (const { name } of this.types[enumName][subgraphName].src.enumValues) {
        if (!routes.has(name)) { unroutable.push(`${name} (${subgraphName})`) }
      }
    }
    if (unroutable.length > 0) {
      throw new Error(`Cannot compose ${where}: enum ${enumName} has values declared by subgraphs that do not publish the field, ${unroutable.join(', ')}`)
    }

    return { argName, routes }
  }

  createRoutedResolver ({ typeName, fieldId, argName, routes }) {
    return async (parent, args, context, info) => {
      const value = args?.[argName]
      const subgraphName = routes.get(value)
      if (!subgraphName) {
        throw new Error(`No subgraph resolves ${fieldId} for ${argName}: ${String(value)}`)
      }

      const fieldSrc = this.fields[fieldId][subgraphName].src
      return this.runResolver({ typeName, subgraphName, fieldSrc, parent, args, context, info })
    }
  }
...
// the enum type name of an argument declared as `Enum` or `Enum!`; lists and other kinds do not
// route, a call has to land on exactly one subgraph
function enumArgTypeName (type) {
  const t = type.kind === 'NON_NULL' ? type.ofType : type
  return t.kind === 'ENUM' ? t.name : undefined
}

// an introspection type reference as SDL, for example `[Book!]!`
function printTypeRef (type) {
  if (type.kind === 'NON_NULL') { return `${printTypeRef(type.ofType)}!` }
  if (type.kind === 'LIST') { return `[${printTypeRef(type.ofType)}]` }
  return type.name
}

// a field's return type and argument types as SDL, argument order and defaults aside, to compare
// across subgraphs
function printFieldSignature (field) {
  const args = (field.args ?? [])
    .map(arg => `${arg.name}: ${printTypeRef(arg.type)}`)
    .sort()
  return `(${args.join(', ')}): ${printTypeRef(field.type)}`
}
