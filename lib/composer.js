...
class Composer {
  ...
  async compose (options) {
    this.schemas = await this.fetchSubgraphSchemas(options)
  ...
  async fetchSubgraphSchemas (options) {
    const subgraphs = Array.from(this.subgraphs.values())

    const requests = subgraphs.map(({ server }) => {
      return fetchSubgraphSchema({ server, ...options })
    })
