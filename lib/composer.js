...
class Composer {
    ...
    for (let i = 0; i < subgraphs.length; ++i) {
      ...
      if (isObject(entities)) {
        ...
        for (let j = 0; j < entityNames.length; ++j) {
          ...
          // TODO fix keys definition: must identify the identity + add other keys
          // on entities, default key = ['id']
          // can detect entities by introspection?

          // validateFunction(args, `subgraphs[${i}].entities.${name}.args`)
          // validateString(referenceListResolverName, `subgraphs[${i}].entities.${name}.referenceListResolverName`)
          if (!Array.isArray(keys)) {
            throw new TypeError(`subgraphs[${i}].entities.${name}.keys must be an array`)
          }
          for (let k = 0; k < keys.length; ++k) {
            validateString(keys[k].field, `subgraphs[${i}].entities.${name}.keys[${k}].field`)
            validateString(keys[k].type, `subgraphs[${i}].entities.${name}.keys[${k}].type`)
          }
  ...
  #isFollowupSolved (followup, merged) {
    ...
    // TODO use result merged map, no result
    // result can be null

    console.log('#isFollowupSolved')

    console.log('followup.keys')
    console.log(followup.keys)

    // TODO use followup.origin
    ...
    return followup.fields.some((f) => {
      return (f.schemaNode.name in merged.data)
    })
  }
}
...
function mergeResults (query, partialResult, response) {
  // TODO refactor, this fn is hard to read, too long, does too much
  ...
  if (mergeAt === null) {
    ...
    // TODO! not by index!
    // match by query.origin.results, query.origin.args
    // TODO test with nullable values
    // TODO test with rotated results

    if (!query.keys) {
      // TODO warn('something wrong in config, missing entities ids')
    }

    // TODO support composite key
    const key = query.keys[0].field
    // TODO multiple entities in same query?
    const entity = query.keys[0].type
    const originKey = query.originKeys[0].field

    // TOOD review, cut useless info
    if (!map[entity]) {
      map[entity] = {
    ...
    for (let i = 0; i < result.length; i++) {
      const r = result[i]
      const rowKey = r[key]
      map[entity].keys.push(rowKey)
      const row = mergeAt.find(r => r[originKey] === rowKey)
      if (!row) { continue }

      for (const [k, v] of Object.entries(r)) {
        // TODO? map[entity].fields.puhs(k)
        row[k] = v
      }
    }
  ...
  // TODO build map and use it to check followups
  return { data: mergeAt, map }
}
