...
class Composer {
      ...
      if (entities) {
        ...
        for (let j = 0; j < entityNames.length; ++j) {
          ...
          for (let k = 0; k < many.length; ++k) {
            ...
            if (m.fkey) {
              validateString(m.subgraph, `subgraphs[${subgraphName}].entities.${name}.many[${k}].fkey`)
            }

            if (!m.subgraph) {
              m.subgraph = subgraphName
            } else {
              validateString(m.subgraph, `subgraphs[${subgraphName}].entities.${name}.many[${k}].subgraph`)
            }
  ...
  #createResolver ({ type, field, fieldName, subgraph }) {
    return async (parent, args, contextValue, info) => {
      ...
      const schemaNodeTypeName = this.#types.get(schemaNodeType.name)
      const query = new QueryBuilder({
        node,
        schemaNode,
        path: [fieldName],
        type: schemaNodeTypeName,
        fields: [],
  ...
  async #runQuery (ctx, query) {
    ...
    this.#logger.debug({ subgraph: query.subgraph.name, query: text }, 'run subgraph query')
    const data = await makeGraphqlRequest(text, query.subgraph.server)

    this.#logger.debug({ query: text, data }, 'query result')
    mergeResults(query, ctx.result, data)
    this.#logger.debug({ merged: ctx.result }, 'merged result')
