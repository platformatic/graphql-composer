...
class Composer {
  ...
  /**
   * resolve a Query or Mutation field that more than one subgraph publishes, by `onConflict`;
   * unset warns and takes the last subgraph's resolver while `mergeTypes` keeps the first
   * subgraph's enum values, so a call can validate against one subgraph and run against another
   */
  ...
  // a routed call is validated against the merged schema, which carries the first publisher's
  // signature, then sent to whichever subgraph owns the value: a valid call must be valid there too
  ...
  // an argument the client leaves out is sent without a value, so the receiving subgraph applies
  // its own default: they must agree. The routing argument is exempt: a subgraph can only default
  // it to a value it owns, and a call left to the merged (first) default is routed there
  ...
  sharedFieldDiscriminator (fieldId, subgraphNames, where) {
    ...
    // the merged enum spans every subgraph declaring it, not only the field's publishers: a value
    // from elsewhere passes validation with no subgraph to go to
