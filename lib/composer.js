...
const { unwrapFieldTypeName, objectDeepClone, pathJoin, schemaTypeName, excludeObjectKeys } = require('./utils')
...
class Composer {
  ...
  async runResolver ({ typeName, subgraphName, fieldSrc, parent, args, context, info }) {
    ...
    const result = await this.runQueries(queries, order, excludeObjectKeys(context.reply.request.headers, ['content-type', 'content-length']))
