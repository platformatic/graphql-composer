...
class Composer {
  ...
  constructor(options) {
    // TODO(cjihrig): Input validation on option values.
    const {
      queryTypeName = 'Query',
    ...
    // TODO(cjihrig): Can't clone subgraphs due to functions.
    // this.#subgraphs = structuredClone(subgraphs);
    this.#types = new Map();
  ...
  #buildArguments(queryInfo, ctx, node) {
    ...
    if (typeof adapter === 'function') {
      ...
      // TODO(cjihrig): Validate returned argsobject.
        ...
        if (selectionSchemaNode) {
          if (!field.subgraphs.has(upstream)) {
            ...
            if (existing === undefined) {
              ...
              const upstream = Array.from(field.subgraphs)[0];
              const typeName = type.schemaNode.name;
              // TODO(cjihrig): Throw if this doesn't exist.
              const resolverName = upstream.entities[typeName].referenceListResolverName;

              const adapter = upstream.entities[typeName].adapter;
