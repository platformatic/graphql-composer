...
const { toQueryArgs, transitiveKeys, keySelection, traverseResult } = require('./utils')
...
class QueryBuilder {
  ...
  partialResults (result) {
    ...
    for (let i = 0; i < this.path.length; ++i) {
      ...
      result = traverseResult(result, path)
    }
      ...
      if (s.kind === 'FragmentSpread' || s.kind === 'InlineFragment') {
        ...
        if (selectionSchemaNode) {
          if (!field.subgraphs.has(this.subgraph)) {
            const index = this.followupIndex(node, field)
            let followup = ctx.followups.get(index)

            if (!followup) {
              keyFields = this.getKeyFields(type)
              followup = this.createFollowup(ctx, schemaNode, node, type, field, keyFields)
              if (followup) {
  ...
  followupSubgraph (field) {
    ...
    return Array.from(field.subgraphs)[0]
  }

  createFollowup (ctx, schemaNode, node, type, field, keyFields) {
    const typeName = type.schemaNode.name

    const nextSubgraph = this.followupSubgraph(field)
  ...
  // TODO memoize
  followupIndex (node, field) {
    const subgraph = this.followupSubgraph(field)
    return `src:${node.name.loc.source.body}#name:${node.name.value}#sg:${subgraph.name}@${subgraph.server.host}`
  }
}
